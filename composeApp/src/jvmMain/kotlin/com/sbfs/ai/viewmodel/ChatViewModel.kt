        ...
        return buildString {
            when (agentMode) {
                AgentMode.DEVELOPER -> {
                    ...
                    append(
                        "Ты можешь самостоятельно читать и записывать файлы проекта используя доступные инструменты (tools):\n" +
                        "- read_project_file(project_path, file_path) - чтение содержимого файла\n" +
                        "- write_project_file(project_path, file_path, content) - запись содержимого в файл\n" +
                        "- list_project_files(project_path, [file_extension]) - список файлов в проекте\n" +
                        "- create_project_file(project_path, file_path) - создание нового файла\n" +
                        "- delete_project_file(project_path, file_path) - удаление файла\n\n" +
                        "Подходы к работе с файлами:\n" +
                        "- Для чтения маленьких файлов - читай целиком\n" +
                        "- Для больших файлов - читай по частям, указывая конкретные секции\n" +
                        "- Для создания/изменения файлов - используй write_project_file\n" +
                        "- Для навигации по проекту - используй list_project_files\n\n" +
